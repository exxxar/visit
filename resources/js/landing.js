/* ============ ВИЗИТ ДОНЕЦК · landing.js ============
   Лендинг = Blade SSR + этот файл как гидратация и интерактив.
   · Данные: window.LANDING = { places: [...], districts: [...] } (инъектит LandingController)
   · Формы: POST /api/v1/applications · /api/v1/leads · /api/v1/subscribe
   · Корневые категории в БД сидируются со slug = ключам CATS (food|coffee|bars|…)
   · Карточка места на карте ведёт на серверную страницу /place/{slug}
==================================================== */
...
document.addEventListener('click', e => {
    ...
    if (d) { e.preventDefault(); toast('Раздел в разработке — демо-версия гида ✨'); }
});
...
PLACES.forEach(p => {
    ...
    b.title = p.name;
    b.onclick = e => { e.stopPropagation(); openCard(p); };
    world.appendChild(b);
    p.el = b;
});
...
function applyState() {
    PLACES.forEach(p => {
        ...
        p.el.classList.toggle('dim', !bySearch || (district !== 'all' && p.d !== district));
    });
}
...
$('#mapView').addEventListener('click', e => {
    if (!e.target.closest('.pin,.map-card')) closeCard();
});
...
function clearSearch() {
    ...
    const qi = $('#quickForm input'); if (qi) qi.value = '';
}

function setFilter(f) {
    filter = f;
    clearSearch();   // ← поиск гаснет вместе с подсветкой
...
$('#districtSel').onchange = e => {
    ...
    clearSearch();   // ← то же при смене района
    applyState();
};
applyState();
...
$('#quickForm').addEventListener('submit', e => { e.preventDefault(); runSearch(e.target.querySelector('input').value); });
$('#navSearch').onclick = () => { location.hash = '#top'; setTimeout(() => $('#searchInput').focus({ preventScroll: true }), 600); };
...
/* ============ РАЙОНЫ ============ */
/* дизайн-мета (цвета, чипы, якоря тултипа); счётчики и фото — из БД */
const D_META = {
...
Object.entries(DISTRICTS).forEach(([name, d], i) => {
    ...
    el.onmouseenter = () => { const p = $(`.d-area[data-d="${name}"]`); p && p.classList.add('hot'); };
    el.onmouseleave = () => { const p = $(`.d-area[data-d="${name}"]`); p && p.classList.remove('hot'); };
...
addEventListener('keydown', e => {
    if (e.key === 'Escape') { $$('.modal.open').forEach(closeModal); closeViewer(); }
});
...
$('#mNext').onclick = async () => {
    if (step === 0 && (!$('#orgName').value.trim() || !$('#orgCat').value)) { toast('Заполните название и категорию'); return; }
    if (step === 1 && !$('#personName').value.trim()) { toast('Укажите имя контактного лица'); return; }
    if (step === 3) {
        if ($$('#addModal .req-chk').filter(c => !c.checked).length) { toast('Подтвердите все условия размещения'); return; }
        ...
        if (!res.ok) { toast(res.error); goStep(0); return; }
        $('#addFlow').hidden = true;
...
$('#leadSubmit').onclick = async () => {
    ...
    if (chks.filter(c => !c.checked).length) { toast('Отметьте согласие на обработку данных и политику'); return; }
...
function goPage(i, dir) {
    ...
    pages.forEach((p, k) => { p.classList.toggle('cur', k === cur); p.classList.remove('anim-n', 'anim-p'); });
    const c = pages[cur];
    void c.offsetWidth;
...
$('#vZoomIn').onclick = () => { zoom = Math.min(1.6, zoom + .15); $('#vStage').style.setProperty('--zoom', zoom); };
$('#vZoomOut').onclick = () => { zoom = Math.max(.7, zoom - .15); $('#vStage').style.setProperty('--zoom', zoom); };
$('#vFull').onclick = () => { document.fullscreenElement ? document.exitFullscreen() : $('#viewer').requestFullscreen(); };
$('#vShare').onclick = async () => {
    try { await navigator.share({ title: 'ВИЗИТ ДОНЕЦК — печатная версия', url: location.href }); }
    catch (e) { toast('Ссылка скопирована: визит-донецк.рф'); }
};
...
$('#geoBtn').onclick = function () {
    ...
    if (navigator.geolocation) {
        try { navigator.geolocation.getCurrentPosition(() => {}, () => {}, { timeout: 1200 }); } catch (e) {}
    }
    setTimeout(() => {
        this.hidden = true;
